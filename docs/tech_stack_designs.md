...
│   ├── decisions.md               # Technical decisions (this file)
...
│   ├── alert_thresholds.yml       # Alert rules config
│   ├── ingestion_schedule.yml     # Ingestion window settings
...
│   │   ├── database_connection.py # PostgreSQL/TimescaleDB client
...
│   │   ├── kafka_publisher.py     # Publish to Kafka
│   │   ├── scheduler.py           # Cron job for 8-9 AM UTC window
│   │   └── tests/
│   │       ├── unit/
│   │       │   ├── test_data_normalizer.py
│   │       │   └── test_kafka_publisher.py
│   │       └── integration/
│   │           └── test_websocket_to_kafka.py
│   │
│   ├── analytics/                 # Stream processing & metrics
│   │   ├── Dockerfile
│   │   ├── requirements.txt
│   │   ├── README.md
│   │   ├── main.py                # Entry point
│   │   ├── kafka_consumer.py      # Consume market-data-raw
│   │   ├── metrics_calculator.py  # Volatility, MDD, P&L calcs
│   │   ├── alert_engine.py        # Rule evaluation & triggering
│   │   ├── redis_state_manager.py # Manage rolling window state
│   │   ├── database_writer.py     # Persist events to TimescaleDB
│   │   └── tests/
│   │       ├── unit/
...
│       ├── routes/
│       │   ├── __init__.py
│       │   ├── metrics.py         # GET /api/metrics (SSE)
│       │   ├── alerts.py          # GET /api/alerts
│       │   ├── replay.py          # GET /api/replay?alert_id=...
...
│       │   ├── metric.py          # Pydantic models for metrics
│       │   └── replay.py          # Pydantic models for replay
│       ├── services/
│       │   ├── redis_subscriber.py # Subscribe to Redis Pub/Sub
│       │   ├── database_query.py   # Query TimescaleDB for replay
│       │   └── sse_manager.py      # Manage SSE connections
│       └── tests/
│           ├── unit/
│           │   ├── test_auth.py
│           │   └── test_models.py
...
│   │   │   ├── PortfolioSummary.tsx # MtM, P&L display
│   │   │   ├── MetricsDisplay.tsx  # Volatility, MDD widgets
│   │   │   ├── AlertLog.tsx        # Scrolling alert list
│   │   │   ├── EventReplay.tsx     # Replay chart modal
│   │   │   └── AuthProvider.tsx    # Firebase auth wrapper
│   │   ├── services/
│   │   │   ├── api_client.ts       # API fetch wrapper
│   │   │   ├── sse_client.ts       # SSE connection handler
│   │   │   └── firebase_auth.ts    # Firebase SDK integration
│   │   ├── hooks/
│   │   │   ├── useMetricsStream.ts # SSE hook for metrics
│   │   │   ├── useAlerts.ts        # Alert fetching hook
│   │   │   └── useReplay.ts        # Replay data fetching
│   │   ├── utils/
...
│   └── e2e/
│       ├── test_full_pipeline.py  # Ingestion → Analytics → API → Frontend
│       ├── test_alert_workflow.py # Alert trigger → storage → delivery
│       └── test_replay_accuracy.py # Replay data correctness
│
├── scripts/                       # Utility scripts
│   ├── setup_local_env.sh         # Initialize Docker Compose
│   ├── seed_test_data.py          # Populate DB with sample data
│   ├── run_tests.sh               # Run all tests
│   └── deploy_cloud.sh            # Deploy to cloud (Render/Vercel)
│
├── .github/                       # CI/CD workflows
│   └── workflows/
│       ├── test.yml               # Run tests on PR
│       ├── deploy.yml             # Deploy on merge to main
│       └── security_scan.yml      # Dependabot + container scan
│
